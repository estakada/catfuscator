...
bool vm_engine::virtualize(const std::vector<obfuscator::instruction_t>& instructions,
	...
	// CRITICAL: pass the PLAINTEXT bytecode to dispatcher.generate() for the
	// anti-tamper checksum, because at runtime the bytecode is decrypted
	// in-place by VM_ENTER and the checksum-verify in VM_EXIT runs on the
	// (now-plaintext) in-memory bytes. If we checksum the encrypted bytes
	// here, runtime XOR-of-bytes is taken over plaintext, so the two values
	...
	std::vector<uint8_t> dispatcher_code;
	if (!dispatcher.generate(dispatcher_code, encrypt_key, ENCRYPT_KEY_SIZE, bytecode_size,
		imm_xor_key, &settings, context_seed, image_base, false, bytecode_plain_copy.data())) {
